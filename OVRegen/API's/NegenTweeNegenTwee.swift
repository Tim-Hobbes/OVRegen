    ...
    func getRoutes (from: String, to: String, departureTime: Date, completion: @escaping ([Journey]) -> ()) {
        self.getLocation(query: from) { (fromId) in
            self.getLocation(query: to) { (toId) in
                ...
                let urlString = "https://api.9292.nl/0.1/journeys?before=1&sequence=1&byFerry=true&bySubway=True&byBus=true&byTrain=true&lang=nl-NL&searchType=departure&interchangeTime=standard&after=5&from=\(fromId.id)&to=\(toId.id)&dateTime=\(dateFormatter.string(from: departureTime))"
